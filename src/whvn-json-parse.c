...
void *whvn_json_parse_thumbs(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    if(!str_cmp(key.s, str("small"))) json_fix_str(&thumbs->small, v.s);
    if(!str_cmp(key.s, str("large"))) json_fix_str(&thumbs->large, v.s);
    if(!str_cmp(key.s, str("original"))) json_fix_str(&thumbs->original, v.s);
    return 0;
}
...
void *whvn_json_parse_tag(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    if(!str_cmp(key.s, str("name"))) json_fix_str(&tag->name, v.s);
    if(!str_cmp(key.s, str("alias"))) json_fix_str(&tag->alias, v.s);
    if(!str_cmp(key.s, str("category_id"))) tag->category_id = z;
    if(!str_cmp(key.s, str("category"))) json_fix_str(&tag->category, v.s);
    if(!str_cmp(key.s, str("purity"))) whvn_json_value_parse_purity(v, &tag->purity);
    if(!str_cmp(key.s, str("created_at"))) json_fix_str(&tag->created_at, v.s);
    return 0;
}
...
void *whvn_json_parse_avatar(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    if(!str_cmp(key.s, str("200px"))) json_fix_str(&avatar->px200, v.s);
    if(!str_cmp(key.s, str("128px"))) json_fix_str(&avatar->px128, v.s);
    ...
    if(!str_cmp(key.s, str("username"))) json_fix_str(&uploader->username, v.s);
    if(!str_cmp(key.s, str("group"))) json_fix_str(&uploader->group, v.s);
    return 0;
}
...
void *whvn_json_parse_wallpaper_info(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    if(val && user) {
        ...
        if(!str_cmp(key.s, str("id"))) json_fix_str(&info->id, v.s);
        else if(!str_cmp(key.s, str("url"))) json_fix_str(&info->url, v.s);
        else if(!str_cmp(key.s, str("short_url"))) json_fix_str(&info->short_url, v.s);
        ...
        else if(!str_cmp(key.s, str("source"))) json_fix_str(&info->source, v.s);
        ...
        else if(!str_cmp(key.s, str("resolution"))) json_fix_str(&info->resolution, v.s);
        else if(!str_cmp(key.s, str("ratio"))) json_fix_str(&info->ratio, v.s);
        else if(!str_cmp(key.s, str("file_size"))) info->file_size = (unsigned long)z;
        else if(!str_cmp(key.s, str("file_type"))) json_fix_str(&info->file_type, v.s);
        else if(!str_cmp(key.s, str("created_at"))) json_fix_str(&info->created_at, v.s);
        else if(!str_cmp(key.s, str("path"))) json_fix_str(&info->path, v.s);
    }
    return 0;
}
...
void *whvn_json_parse_tag_info(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    if(v.id == JSON_NUMBER) str_as_size(v.s, &z, 10);
    if(!str_cmp(key.s, str("id"))) tag->id = z;
    else if(!str_cmp(key.s, str("name"))) json_fix_str(&tag->name, v.s);
    else if(!str_cmp(key.s, str("alias"))) json_fix_str(&tag->alias, v.s);
    else if(!str_cmp(key.s, str("category_id"))) tag->category_id = z;
    else if(!str_cmp(key.s, str("category"))) json_fix_str(&tag->category, v.s);
    else if(!str_cmp(key.s, str("purity"))) whvn_json_value_parse_purity(v, &tag->purity);
    else if(!str_cmp(key.s, str("created_at"))) json_fix_str(&tag->created_at, v.s);
    return 0;
}
...
void *whvn_json_parse_user_settings(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    size_t z = 0;
    if(v.id == JSON_NUMBER) str_as_size(v.s, &z, 10);
    if(!str_cmp(key.s, str("thumb_size"))) json_fix_str(&settings->thumb_size, v.s);
...
void *whvn_json_parse_user_collection(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    else if(!str_cmp(key.s, str("label"))) json_fix_str(&collection->label, val->s);
...
void *whvn_json_parse_meta(void **user, JsonParseValue key, JsonParseValue *val) {
    ...
    else if(!str_cmp(key.s, str("query"))) json_fix_str(&meta->query, val->s);
